...
//			var bumpablesAtNewLocation = level.GetEntitiesAt(newLocation).Where(e => e.Has<OnBump>());
...
//		}
//
//		public static void MoveEntity(Entity entity, Point newLocation) {
//			Contract.Requires<ArgumentNullException>(entity != null, "entity");
...
//			if (entity.Get<Location>().Level.IsWalkable(newLocation)) {
//				entity.Get<Location>().Point = newLocation;
...
////			Contract.Requires<ArgumentNullException>(entity != null, "entity");
////
////			Point newLocation = entity.Get<Location>().Location + direction;
////
////			var level = entity.Get<Location>().Level;
////
////			// first we see if there are any entities that we "bump" into when we walk onto new location
////			var bumpablesAtNewLocation = level.GetEntitiesAt(newLocation, typeof(OnBump)).ToList();
////			bool blockedMovement = false;
////
////			foreach (var b in bumpablesAtNewLocation) {
////				if (b.Get<OnBump>().Action(entity, b) == OnBump.BumpResult.BlockMovement) {
...
////			var actorsAtNewLocation = level.GetEntitiesAt(newLocation, typeof(DefendComponent)).ToList();
////
...
////
////			// finally move onto the new location
////			if (level.IsWalkable(newLocation)) {
////				entity.Get<Location>().Location = newLocation;
////				entity.Get<ActionPoint>().ActionPoints -= World.SpeedToActionPoints(World.DEFAULT_SPEED);
////
////				// check if we're near anything
