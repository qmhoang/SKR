...
namespace SkrGame.Actions.Skills {
	public class LockpickAction : AbstractItemAction {
		...
		public LockpickAction(Entity entity, Entity item, Entity feature) : base(entity, item) {
			...
			_lengthRequired = new TimeSpan(0, 0, 1, 0);
		}
		...
		private TimeSpan _lengthRequired;

		public override ActionResult OnProcess() {
			if (_feature.Get<LockedFeature>().Status == LockStatus.Opened) {
				World.Log.AbortedFormat("{0} isn't locked.", Identifier.GetNameOrId(_feature));
				return ActionResult.Aborted;
			}

			var person = Entity.Get<Creature>();
			double easeOfLock = Item.Get<Lockpick>().Quality + person.Skills["skill_lockpicking"].Value - _feature.Get<LockedFeature>().Quality;
					               ...
					               e =>
					               	{
					               		...
					               		double agiDiff = Item.Get<Lockpick>().Quality +
					               		                 ...
					               		                 _feature.Get<LockedFeature>().Quality +
					               		                 bonus;

					               		Logger.InfoFormat("{0} attempts in Lockpicking/Agility on {1} (needs:{2:0.00}, rolled:{3:0.00}, difficulty: {4:0.00}%)",
					               		                  EntityName, Identifier.GetNameOrId(_feature), agiDiff, agiRoll, World.ChanceOfSuccess(agiDiff));

					               		if (agiRoll <= agiDiff) {
					               			_feature.Get<LockedFeature>().Status = LockStatus.Opened;
					               			World.Log.GoodFormat("{0} succeeds in lockpicking {1}.", EntityName, Identifier.GetNameOrId(_feature));
					               			return ActionResult.Success;
					               		}
					               		World.Log.FailFormat("{0} fails in lockpicking {1}.", EntityName, Identifier.GetNameOrId(_feature));
